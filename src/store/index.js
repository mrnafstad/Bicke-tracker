...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    toggleStationInfo: (state) =>
      (state.showStationInfo = !state.showStationInfo),
      ...
      await fetch(
        ...
        .then((data) => {
          data.data.stations.forEach((element) => {
            var station = this.state.stations.find((station) => {
              return station.station_id === element.station_id;
            });
  ...
  getters: {
    stationsFiltered: (state) => {
      ...
      if (!state.onlyAvailable && !state.showFull) {
        returnStat = state.stations.filter((stat) => {
        ...
        returnStat = state.stations.filter((stat) => {
          return stat.available < stat.capacity && stat.available > 0;
        });
      } else if (state.onlyAvailable && state.showFull) {
        returnStat = state.stations.filter((stat) => {
          return stat.available < stat.capacity;
        });
      } else {
        returnStat = state.stations;
      }
