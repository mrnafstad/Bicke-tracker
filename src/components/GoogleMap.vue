<template>
  <v-container style="padding: 0px">
    <v-overlay :value="showAppInfo">
      <v-card raised light>
        <v-list>
          <v-list-item>This is a simple tracking app for Oslo city bikes</v-list-item>
          <v-list-item>Stationdata is updated every minute</v-list-item>
          <v-list-item>Click any station to get current data</v-list-item>
          <v-list-item>Toggle filters in the menu in the top left corner</v-list-item>
        </v-list>
      ...
      <v-card raised light>
        <v-list-item two-line>
          <v-list-item-content>
            ...
            <v-list-item-subtitle>Ledig: {{ currentInfo.available }}/{{ currentInfo.capacity }}</v-list-item-subtitle>
            <v-spacer></v-spacer>
    ...
    <gmap-map :center="center" :zoom="12" style="width:100%;  height: 90vh">
      <gmap-marker
        :key="index"
        ...
        :clickable="true"
      ></gmap-marker>
      <gmap-marker :position="center" :clickable="true" @click="user.show = !user.show"></gmap-marker>
    </gmap-map>
  </v-container>
</template>
